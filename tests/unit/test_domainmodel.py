...
def test_user_add_remove_reviews():
    ...
    adding_user = User("User", "jkfg934dj")
    game = Game(1, "Domino Game")
    review1 = Review(user, game, 3, "Great game!")
    review2 = Review(user, Game(2, ""), 4, "Superb game!")
    review3 = Review(user, Game(3, ""), 2, "Boring game!")
    assert len(user.reviews) == 3
    assert len(adding_user.reviews) == 0
    adding_user.add_review(review1)
    adding_user.add_review(review2)
    adding_user.add_review(review3)
    assert len(adding_user.reviews) == 3
    assert len(user.reviews) == 3
    assert user.reviews == [review1, review2, review3]
    assert adding_user.reviews == [review1, review2, review3]
...
def test_review_eq():
    ...
    review2 = Review(user, Game(2, ""), 4, "Superb game!")
    review3 = Review(user, Game(3, ""), 5, "Boring game!")
    review4 = Review(user, Game(4, ""), 2, "Classic game!")
